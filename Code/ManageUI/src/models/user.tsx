	...
	effects: {
		*getUsers({ payload }, { call, put }) {
			...
			const { success, result } = yield call(
				...createApiAuthParam({
					method: new api.UserApi().getUsers,
					payload: {
						...payload,
		...
		*getUsersToExcel({ payload }, { call, put }) {
			const { success, result } = yield call(
				...createApiAuthParam({
					method: new api.UserApi().getUsersToExcel,
					payload: {
						...payload
					}
				})
		...
		*createOrUpdateUser({ payload }, { call, put }) {
			const { success, result } = yield call(
				...createApiAuthParam({
					method: payload.id ? new api.UserApi().update : new api.UserApi().create,
					payload: payload
				})
			);
			if (success) {
				if (!payload.id) {
					notification.success({
						...
						description: (
							<span>
								您已成功添加用户<strong>{payload.name}</strong>
							</span>
						)
					});
					yield put({
					...
					notification.success({
						...
						description: (
							<span>
								您已成功修改用户 <strong> {payload.name} </strong>的信息
							</span>
						)
					});
					yield put({
		...
		*getRoles({}, { call, put }) {
			const { success, result } = yield call(
				...createApiAuthParam({
					method: new api.UserApi().getRoles
				})
			);
			if (success) {
				yield put({
		...
		*getOrganizationUnits({ payload }, { call, put }) {
			const { success, result } = yield call(
				...createApiAuthParam({
					method: new api.OrganizationUnitApi().getOrganizationUnits,
		...
		*batchDeleteUser({ payload }, { call, put }) {
			const { success, result } = yield call(
				...createApiAuthParam({
					method: new api.UserApi().batchDelete,
					payload: { value: payload.value }
				})
			);
			if (success) {
				notification.success({
					...
					description: (
						<span>
							您已成功删除 <strong> {payload.users[0].name} </strong>等<strong>{payload.users.length}</strong>{' '}
							位用户
						</span>
					)
				});
		...
		*batchUnlockUser({ payload }, { call, put }) {
			yield put({
				...
				payload: {
					...payload,
					operate: '解锁',
					serverUrl: new api.UserApi().batchUnlockUser
				}
			});
		},
		*batchActiveUser({ payload }, { call, put }) {
			yield put({
				...
				payload: {
					...
					operate: '启用',
					isActive: true,
					serverUrl: new api.UserApi().batchActiveUser
				}
			});
		},
		...
		*deleteUser({ payload }, { call, put }) {
			const { success, result } = yield call(
				...createApiAuthParam({
					method: new api.UserApi()._delete,
		...
		*getUserForEdit({ payload }, { call, put }) {
			const { success, result } = yield call(
				...createApiAuthParam({
					method: new api.UserApi().getUserForEdit,
					payload: payload
				})
			);
			if (success) {
		...
		*getUserPermissionsForEdit({ payload }, { call, put }) {
			const { success, result } = yield call(
				...createApiAuthParam({
					method: new api.UserApi().getUserPermissionsForEdit,
					payload: payload
				})
			);
			if (success) {
		...
		*updateUserPermissions({ payload }, { call, put }) {
			const { success, result } = yield call(
				...createApiAuthParam({
					method: new api.UserApi().updateUserPermissions,
					payload: payload
				})
			);
			if (success) {
		...
		*toggleActiveStatus({ payload }, { call, put }) {
			const { success, result } = yield call(
				...createApiAuthParam({
					method: new api.UserApi().toggleActiveStatus,
					payload: { id: payload.id }
				})
			);
			if (success) {
		...
		*unlockUser({ payload }, { call, put }) {
			const { success, result } = yield call(
				...createApiAuthParam({
					method: new api.UserApi().unlockUser,
					payload: { id: payload.id }
				})
			);
			if (success) {
		...
		*resetUserPassword({ payload }, { call, put }) {
			const { success, result } = yield call(
				...createApiAuthParam({
					method: new api.ProfileApi().resetUserPassword,
					payload: { id: payload.id }
				})
			);
			if (success) {
		...
		*changeUserPassword({ payload }, { call, put }) {
			const { success, result } = yield call(
				...createApiAuthParam({
					method: new api.ProfileApi().changeUserPassword,
					payload: {
						userId: payload.id,
		...
		*resetUserSpecificPermissions({ payload }, { call, put }) {
			const { success, result } = yield call(
				...createApiAuthParam({
					method: new api.UserApi().resetUserSpecificPermissions,
					payload: payload
				})
			);
			if (success) {
	...
	subscriptions: {
		setup({ dispatch, history }) {
			return history.listen(({ pathname, state }) => {
				if (pathname.toLowerCase() == '/home/user'.toLowerCase()) {
					dispatch({
						type: 'getUsers',
						payload: {
							current: 1,
