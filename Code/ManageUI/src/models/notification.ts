...
export default {
	...
	effects: {
		...
		*setNotificationAsRead({ payload }, { call, put, select }) {
			const { success, result } = yield call(
				...createApiAuthParam({
					method: new api.NotificationApi().setNotificationAsRead,
					payload: payload
				})
			);
			if (success) {
	...
	subscriptions: {
		setup({ dispatch, history }) {
			return history.listen(({ pathname, state }) => {
				if (
					pathname.toLowerCase() == '/home/notification'.toLowerCase() ||
					pathname.toLowerCase() == '/teacher/notification'.toLowerCase()
				) {
					dispatch({
						type: 'getUserNotifications',
						payload: {
